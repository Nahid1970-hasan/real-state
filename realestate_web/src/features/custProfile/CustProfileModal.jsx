...
import { initLoader, loadCustProfile, updateCustProfile } from "./cust_profile_slice";
import { Formik } from "formik";
import { Flex } from "../../components/style/Flex_styled";


export const CustProfileModal = ({ open, setOpen }) => {
    ...
    useEffect(() => {
        set_profile_data({
            fullname:profileLoading?.user?.fullname,
            fathers_name: profileLoading?.user?.fathers_name,
            mothers_name:profileLoading?.user?.mothers_name,
            spouse_name: profileLoading?.user?.spouse_name,
            mobile_no: profileLoading?.user?.mobile_no,
            email:profileLoading?.user?.email,
    ...
    useEffect(() => {
        if (profileLoading.updateLoading == "succeeded") { 
          setTimeout(() => { set_disabled(false); setOpen(false) }, 3000);
        } else if (profileLoading.updateLoading != "idle" && profileLoading.updateLoading != "pending") { 
    ...
    const validate = (Values) => {
        ...
        if (!Values.mobile_no) {
            ...
            errors.mobile_no = t("This is not valid Mobile Number");
          }
        if (Values.email) {
    ...
    const editUserProfile = (values) => {
        let pData = profileLoading?.user || {};
        let data = Object.assign({ ...pData, fullname:values?.fullname,
        ...
        dispatch(updateCustProfile(data));
        set_disabled(true);
