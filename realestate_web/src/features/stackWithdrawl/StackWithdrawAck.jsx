...
import { undateWithdrawlAck, loadWithdrawlInfo, initLoader} from "./stack_withdrawl_slice";

export const SendWithdrawlAckModal = ({
    add,
    open,
    setOpen = () => { },
    data,
}) => { 
    const stckwithdrawl = useSelector((state) => state.stackwithdrawl);
    ...
    useEffect(() => { 
        var MSG  = "Dear "+data.investor_name+" Sir, Your withdrawal request of amount Tk "+data?.amount+" Date: "+data?.request_date+" from "+data?.request_for +" of "+data.project_name+" has been verified.";
        set_pay_data({
            message: MSG,
            email_subject: "Recognition for withdrawal request from " +data?.request_for+" of "+data.project_name, 
            email_body: MSG+" -Thanks"
        });
        set_withdrawl_id(data.withdrawl_id || 0);
        set_user_id(data.user_id || 0);
        set_amount(data.amount || 0);
    }, [data]);

    const SubmitForm = (values) => { 
        values.withdrawl_id = withdrawl_id; 
        values.user_id = user_id;  
        values.amount = amount;  
        dispatch(undateWithdrawlAck(values));
        set_disabled(true);
    }; 

    function set_temp_id_change(event) {
        let tempId = event.target.value;
        var data = stckwithdrawl?.tempList?.find((d) => d.template_id == tempId);
        set_pay_data(data);
    }
    ...
    useEffect(() => {
        if ( stckwithdrawl.addUpdateLoading == "succeeded") {  
            setTimeout(() => {clearForm(); setOpen(false); set_disabled(false);}, 3000);
        } else if (stckwithdrawl.addUpdateLoading != "pending" && stckwithdrawl.addUpdateLoading != "idle") { 
            setTimeout(() => {set_disabled(false);}, 4000);
        }
    }, [stckwithdrawl.addUpdateLoading]);
                            ...
                            <Flex row>
                                ...
                                <Flex padding="0!important" md={9}> 
                                    ...
                                    >
                                        ...
                                        {
                                            stckwithdrawl?.tempList?.map((d, i) => (
