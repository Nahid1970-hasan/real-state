...
import { loadReleaselInfo, tempLoader, initLoader, loadReleaselInfoInit } from "../features/stackRelease/stack_release_slice";
import { StackReleaseGrid } from "../features/stackRelease/StackReleaseGrid";
...
export const StackReleasePage = () => {
  ...
  useEffect(() => {
    if (stckReleaseData.loading == "pending") {
      setIsLoading(true)
    } else if (stckReleaseData.loading != "idle") {
      setTimeout(() => { setIsLoading(false) }, 2000);
    }
  }, [stckReleaseData.loading]);

  useEffect(() => {
    if (stckReleaseData.addUpdateLoading == "pending") {
      setIsLoading(true)
    }
    else if (stckReleaseData.addUpdateLoading == "succeeded") {
      setIsLoading(false);
      dispatch(loadReleaselInfoInit());
      setTimeout(() => { dispatch(initLoader()); }, 5000);
    } else if (stckReleaseData.addUpdateLoading != "idle") {
      setTimeout(() => { dispatch(initLoader()), setIsLoading(false) }, 5000);
    }
  }, [stckReleaseData.addUpdateLoading]);

  useEffect(() => {
    if (stckReleaseData.tempLoading == "pending") {
      setIsLoading(true)
    } else if (stckReleaseData.tempLoading != "pending" && stckReleaseData.tempLoading != "idle") {
      setIsLoading(false); dispatch(tempLoader());
    }
  }, [stckReleaseData.tempLoading]);
    ...
    if (isvalid) {
      ...
      if (searchVal) {
        if (date) {
          ...
          dispatch(loadReleaselInfo(data));
